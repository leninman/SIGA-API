...
import com.tecnodestreza.siga.repo.IAnnioEscolarRepo;
...
import com.tecnodestreza.siga.repo.INotasParRepo;
...
public class INotasServiceImpl implements INotasService {
    ...
    Optional<Lapso> lapso;

    AnnioEscolar periodoescolar;
        ...
        try {
            ...
            if(notasparciales!=null) {
                for (NotaPar notaPar : notasparciales) {
                    ...
                    lapso = lapsorepo.findById(notaPar.getIdLapso());
                    Float rate=lapso.get().getPorcentaje();
    ...
    public  List<NotaWrapper> obtenerNotas(List<NotaPar> notas){
        ...
        for (NotaPar notaPar : notas) {
            ...
            notawrapper.setPeriodo(lapso.get().getPeriodo());

            notawrapper.setIdLapso(lapso.get().getIdLapso());

            notawrapper.setLapsodescripcion(lapso.get().getDescripcion());
